...
//        Optional<Article> articleEntity= articleRepository.findById(id);
    ...
    public String update(ArticleForm form) {
        ...
        // 3. 수정 결과 페이지를 리다이렉트 해준다.
        return "redirect:/articles/" + articleEntity.getId();
    }

    @GetMapping("/articles/{id}/delete")
    public String delete(@PathVariable Long id, RedirectAttributes rttb) {
        log.info("삭제요청이 들어왔습니다.");
        // 1. 삭제 대상을 가져온다.
        Article articleTarget = articleRepository.findById(id).orElse(null);
        // 2. 대상을 삭제한다.
        if (articleTarget != null) {
            articleRepository.delete(articleTarget);
            rttb.addFlashAttribute("msg","삭제가 완료 되었습니다.");
        }
        // 결과 페이지로 리다이렉트 한다.
